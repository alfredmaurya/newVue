<template>
  <q-page :style="!isMobile ? 'padding-top: 70px' : ''">
    <q-page-sticky expand position="top" v-if="!isMobile">
      <q-toolbar class="bg-white text-dark q-pa-none" inset>
        ...
        <q-breadcrumbs class="text-grey col-auto" gutter="sm" separator="/">
          ...
          />
          <q-breadcrumbs-el label="Facility" />
        </q-breadcrumbs>
        <div class="breadcrumb-filters ml-auto flex q-mr-xs">
          <q-btn
            v-if="hasPermission('facility_add')"
            unelevated
            color="primary"
          ...
          >
            <q-list class="dropdown-sm action-dropdown-box">
              <q-item
                clickable
                v-close-popup
                :to="{ name: 'Facility', params: { id: props.row.id } }"
              >
                <q-item-section>
                  <q-item-label>
              ...
              <q-item
                v-if="hasPermission('facility_edit')"
                clickable
                v-close-popup
                @click="
                  updateDialogState({
    ...
    <Form v-if="hasPermission(['facility_add', 'facility_edit'])" />
    <Detail />
  </q-page>
</template>
...
export default {
  name: 'PageFacility',
  computed: {
    ...mapGetters('facility', ['pagination', 'listing']),
    localPagination() {
      return this.pagination
    },
  },
  components: {
    Form: () => import('components/facility/form'),
    Detail: () => import('components/facility/detail'),
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    onRequest(props) {
      ...
      this.$store
        .dispatch('facility/getListing', params)
        .then((response) => {
          if (response && response.status) {
            this.$store.commit('facility/SET_LISTING', {
              listing:
            ...
            this.$store.commit('facility/UPDATE_PAGINATION', {
    ...
    resetPagination() {
      this.$store.commit('facility/UPDATE_PAGINATION', this.defaultPagination)
    },
    confirmDelete(target) {
      this.visiConfirm(
        `Are you sure you want to delete "${target.title}" Role`
      ).onOk(() => {
        this.$store
          .dispatch('facility/deleteById', target.id)
          .then((response) => {
            if (response.status) {
