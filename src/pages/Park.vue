<template>
  <q-page :style="!isMobile ? 'padding-top: 70px' : ''">
    <q-page-sticky expand position="top" v-if="!isMobile">
      <q-toolbar class="bg-white text-dark q-pa-none" inset>
        ...
        <q-breadcrumbs class="text-grey col-auto" gutter="sm" separator="/">
          ...
          />
          <q-breadcrumbs-el label="Park" />
        </q-breadcrumbs>
        <div class="breadcrumb-filters ml-auto flex q-mr-xs">
          <q-btn
            v-if="hasPermission('park_add')"
            unelevated
            color="primary"
          ...
          >
            <q-list class="dropdown-sm action-dropdown-box">
              <q-item
                clickable
                v-close-popup
                :to="{ name: 'Park', params: { id: props.row.id } }"
              >
                <q-item-section>
                  <q-item-label>
              ...
              <q-item
                v-if="hasPermission('park_delete')"
                clickable
                v-close-popup
    ...
    <Form v-if="hasPermission(['park_add', 'park_edit'])" />
    <Detail />
  </q-page>
</template>
...
export default {
  name: 'PagePark',
  components: {
    Form: () => import('components/park/form'),
    Detail: () => import('components/park/detail'),
  },
  computed: {
    ...mapGetters('parks', ['pagination', 'listing']),
    localPagination() {
  ...
  methods: {
    onRequest(props) {
      ...
      this.$store
        .dispatch('parks/getListing', params)
        .then((response) => {
          if (response && response.status) {
            this.$store.commit('parks/SET_LISTING', {
              listing:
            ...
            this.$store.commit('parks/UPDATE_PAGINATION', {
    ...
    resetPagination() {
      this.$store.commit('parks/UPDATE_PAGINATION', this.defaultPagination)
    },
    confirmDelete(target) {
      this.visiConfirm(
        `Are you sure you want to delete "${target.title}" Park`
      ).onOk(() => {
        this.$store.dispatch('parks/deleteById', target.id).then((response) => {
          if (response.status) {
            if (this.$refs.parkGrid) this.$refs.parkGrid.refresh()
  ...
  created() {
    ...
    this.$store.dispatch('common/getAllEmployee')
    this.$store.dispatch('common/getAllFacilitys')
