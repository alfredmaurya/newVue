import { LocationAgent } from 'src/agent'

export async function getListing({}, params = {}) {
  try {
    let { data } = await LocationAgent.get(params)
    return data
  } catch (error) {
    return error
  }
...
export async function addUpdate({ commit }, body = {}) {
  try {
    if (body.id) {
      let { data } = await LocationAgent.update(body)
      if (data.status) commit('ADD_UPDATE_FROM_LISTING', data.data)
      return data
    } else {
      let { data } = await LocationAgent.add(body)
      if (data.status) commit('ADD_UPDATE_FROM_LISTING', data.data)
      return data
    }
  } catch (error) {
...
export async function deleteById({ commit }, id) {
  try {
    let { data } = await LocationAgent.delete(id)
...
export async function detailById({}, id) {
  try {
    let { data } = await LocationAgent.detail(id)
    return data
  } catch (error) {
    return error
  }
