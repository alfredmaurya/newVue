import { TaxAgent } from 'src/agent'

export async function getListing({}, params = {}) {
  try {
    let { data } = await TaxAgent.get(params)
    return data
  } catch (error) {
    return error
  }
...
export async function addUpdate({ commit }, body = {}) {
  try {
    if (body.id) {
      let { data } = await TaxAgent.update(body)
      if (data.status) commit('ADD_UPDATE_FROM_LISTING', data.data)
      return data
    } else {
      let { data } = await TaxAgent.add(body)
      if (data.status) commit('ADD_UPDATE_FROM_LISTING', data.data)
      return data
    }
  } catch (error) {
...
export async function deleteById({ commit }, id) {
  try {
    let { data } = await TaxAgent.delete(id)
...
export async function detailById({}, id) {
  try {
    let { data } = await TaxAgent.detail(id)
    return data
  } catch (error) {
    return error
  }
